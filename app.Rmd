...
pivoted_data_by_year <- data_by_year %>%
        pivot_longer(cols = c("acousticness", 
                     ...
                     names_to = "metric", 
                     values_to = "values")
spotify_rforest_decade <- read_rds("/Users/ankitsanghi/Desktop/Ankit/Carleton/Classes/STAT 220/content/final-project-sanghi-hussain/spotify_rforest_decade.rds")

spotify_rforest_popularity <- read_rds("/Users/ankitsanghi/Desktop/Ankit/Carleton/Classes/STAT 220/content/final-project-sanghi-hussain/spotify_rforest_popularity.rds")
...
Predictions are made using a Random Forest Classification with 500 trees and the default mtry value (square-root of the number of predictors). Specifically, we use two prediction models. In the first one, the musical decade is the the main predictor and in the second one popularity of the song is the main predictor. The musical decades range from the 1920’s to the 2020’s and the popularity variable was grouped into 10 bins (0-10, 11-20, 21-30, 31-40, 41-50, 51-60, 61-70, 71-80, 81-90, 91-100). This was done primarily to reduces the number of classes. For both models we use the same set of predictors: acousticness, danceability, energy, instrumentalness, valence, and speechiness. 
 
The interactive feature bellow allows you to enter the metrics for music that you like to listen to. Our model will tell you which decade your musical tastes belong in as well as how popular a song with these metrics would be. The prediction will take a few seconds, so please be patient.
...
shinyApp(
  ...
  server <- function(input, output) {
     observeEvent(input$go, {
      input_data <- data.frame(acousticness = input$acousticness, danceability = input$danceability,
                                     energy = input$energy, instrumentalness = input$instrumentalness, 
                                     speechiness = input$speechiness, valence = input$valence)
            input_data <- input_data %>%
              mutate(prediction_decade = predict(spotify_rforest_decade, newdata=input_data, type = "response"),
                     prediction_popularity = predict(spotify_rforest_popularity, newdata=input_data, type = "response"))
            output$result <- renderUI(HTML(paste("<h3>Your musical tastes belong in the ",input_data[1, 7] ,"s and would have a popularity between ", input_data[1, 8],"!</h3>", sep = "")))
    })
  },
