...
shinyApp(
  ...
  server <- function(input, output) {
     observeEvent(input$go, {
      input_data <- data.frame(acousticness = input$acousticness, 
      ...
      input_data <- input_data %>%
        mutate(prediction_decade = predict(spotify_rforest_decade, 
                                           newdata=input_data, 
                                           type = "response"),
               prediction_popularity = predict(spotify_rforest_popularity, 
                                               newdata=input_data, 
                                               type = "response"))
      
      output$result <- renderUI(HTML(
        paste("<h3>Your musical tastes belong in the ",
              input_data[1, 7] ,"s and would have a popularity between ", 
              input_data[1, 8],"!</h3>", sep = "")))
    })
  },
